...
if($topmenu_off == 2 || $itemid == 0 ) {
   ...
   if ($text_direction == 1) {
	   ...
	   $document->addStyleDeclaration("
		  ...
		  background-image:url(".$yj_site."/images/".$css_file."/topmenu/arrow_left.gif)!important;
...
$yjsg_js.="var logo_w = $logo_per_width;var site_w = $css_width;var site_f = '$css_font';var sp='$sp';var tp ='$this->template';var compileme =$compileme;";
...
// last to load 
	...
	if(isset($YjsgCustomJS) && is_array($YjsgCustomJS) && !empty($YjsgCustomJS)){
		foreach($YjsgCustomJS as $YjsgCustomJS){
			$document->addScript($YjsgCustomJS);
		}
	}
?>
