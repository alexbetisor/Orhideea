...
// XSS PROTECTION
require_once(YJSGPATH."yjsgcore/yjsg_validate.php");
...
jimport('joomla.filesystem.file');// must include now because of 3.0
...
$comcontent =& $app->getParams('com_content');
 ...
 ($itemid == $component_switch || $turn_component_off == 1) && JFactory::getApplication()->input->get( 'Itemid' ) !=='' ) {	 
	$midblock_off = true;
...
$less_dir 				= YJSGPATH."less".DIRECTORY_SEPARATOR;
$css_dir  				= YJSGPATH."css_compiled".DIRECTORY_SEPARATOR;
...
if(in_array( $itemid, $yjsg_assigements )){
	$sp =JURI::base().'?change_css='.$css_file.'&';
}else{
	$sp =JURI::base().'?';
}
...
// check jbootsrtap state	
		...
		if(JFile::exists($compiler_log)){
			...
			foreach($files['files'] as $filename => $filetime){
				...
				if ((!file_exists($filename)) or filemtime($filename) > $files['updated']) {
