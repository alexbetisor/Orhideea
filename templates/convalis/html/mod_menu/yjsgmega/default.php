...
?>
 ...
 //rasio end
	...
	require( YJSGPATH."html/mod_menu/yjsgmega/yjsg_modhelper.php"); /* top menu only*/
		...
		if($item->level == 1 && in_array($item->id, $path)){
			...
			if($default_menu_style =='3'){
					if($text_direction == 2){
						...
						$document->addCustomTag('
						<style type="text/css">
							li.haschild.active ul.dropline.level1{margin:0;left:0!important;z-index:1!important;}li.haschild.active{position:static;	z-index:1!important;}li.haschild ul.dropline.level1{z-index:2!important;}div.top_menu li.active.is_active ul.dropline.level1{z-index:-1!important;}
						</style>');
			}
			
		}

		$class = ' class="'.$class.'item'.$item->id.$yj_load_mod_class.$lifirst.$lilast.$lilevelclass.'"';
		echo '<li'.$id.$class.'>';
		$group_title='';
		if($item->deeper) {
